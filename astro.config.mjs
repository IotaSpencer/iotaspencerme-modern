...
export default defineConfig({
  ...
  integrations: [mdx(), sitemap(), webcore()],
  adapter: cloudflare({
    platformProxy: {
      enabled: true,
    },
  ...
  vite: {
    ...
    plugins: [
      Icons({
        ...
        // Icon sizing
        scale: 1.2,
        // Scale factor relative to 1em (default: 1.2)
        //
        // Default styling
        defaultStyle: "",
        // CSS styles applied to all icons
        defaultClass: "",
        ...
        // transform: undefined,
        // See [Global Icon Transformation](#global-icon-transformation)
        transform(svg, collection, icon) {
          // apply fill to this icon on this collection
          if (collection === "my-icons" && icon === "account")
            return svg.replace(/^<svg /, '<svg fill="currentColor" ');
          else return svg.replace(/^<svg /, '<svg fill="currentColor" ');
          return svg;
        },
      }),
    ],
